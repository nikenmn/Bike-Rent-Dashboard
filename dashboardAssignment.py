...
with col6:
    ...
    sns.heatmap(correlation_matrix, annot=True, cmap='YlGnBu', fmt=".2f", linewidths=.5, mask=correlation_matrix < 0)
    plt.xlabel('Features')
    plt.ylabel('Features')
    st.subheader("Positive Correlations")
    st.caption("Correlation Between Temperature, Humidity, Windspeed, and Bike Rentals")
    st.pyplot(plt)
